...
[Header("Death/UI")]
    ...
    public GameObject gameOverUI;         // Assign a UI object (e.g., "GAME OVER" text) here
    ...
    public void TakeDamage(int damage, Collider2D damageSource)
    {
        ...
        if (SM.CurrentState is PlayerParry)
        {
            ...
            {
                // Parried something that isn't a known enemy (like a projectile/trap or env damage)
                // No-op: keep the parry visual only.
            }
            return;
        }

        // Check 3: Player is Hit
        // Since HP is 1, this represents death.
        HandleDeath();
    }
    // ------------------------------------------------------------------
    ...
    // Handle 1-HP death for the knight.
    private void HandleDeath()
    {
        ...
        // Start the death animation + game over sequence
    ...
    private IEnumerator DeathSequence()
    {
        ...
        // Freeze on the last frame so it doesn't loop visually
        if (anim != null)
        {
            anim.speed = 0f;
        }

        // Show simple game-over UI placeholder
        if (gameOverUI != null)
        {
        ...
        {
            // Fallback: at least log so we know death completed even if no UI is wired.
            Debug.Log("GAME OVER (no gameOverUI assigned)");
        }
    }
